...
| Trace requests across services | `references/tracing.md` | Enable tracing, SDK instrumentation (preferred) vs automatic (eBPF), what to instrument, provided `OTEL_*` variables, sampling, reading traces with the `list-traces` / `get-trace` MCP tools |
